...
%   2. works-G-H means: "gangster G works at hour H"

writeClauses(K):- 
    initClauseGeneration,
    eachHourEachGangsterAtMostOneTask,
    notTwoDifferentTaskInConsecutiveHours,
    eachTaskIsDone,
    noMorethanKconsecutiveHours(K),
    propagate,
    true,!.
...
notTwoDifferentTaskInConsecutiveHours:-
    ...
    task(T2),
    T1 \= T2,
    writeClause([\+does-G-T1-H, \+does-G-T2-H1]),fail.
notTwoDifferentTaskInConsecutiveHours.

eachTaskIsDone:-
    task(T),
    hour(H),
    findall(does-G-T-H,(gangster(G), available2(G,H)),L),
    needed(T,H,N),
    atLeast(N,L),fail.
eachTaskIsDone.
...
noMorethanKconsecutiveHours(K):-
    available(G,H),
    H1 is H+K,
    findall(\+works-G-H2,(between(H,H1,H2),hour(H2), available2(G,H2)),L),
    K1 is K+1,
    length(L,K1),
    writeClause(L),fail.
noMorethanKconsecutiveHours(_).

propagate:-
    available(G,H),
    task(T),
    writeClause([\+does-G-T-H, works-G-H]),fail.
propagate.
